...
json_stream_output (FILE * file, json_t * root)
{
      ...
      state1:			/* open value */
	{
...
#endif
			buffer_data(file, buffer, &buffer_used_size, sizeof(buffer),
					",", sizeof(",") - 1);
		}
		switch (cursor->type)
		{
		case JSON_STRING:
			/* append the "text"\0, which means 1 + wcslen(cursor->text) + 1 + 1 */
			/* set the new output size */
			temp = json_escape(cursor->text);
			//fprintf (file, "\"%s\"", temp);
#if 0
			if(sizeof(buf) - n < strlen(temp) + 2 + 1)
			{
				fprintf(file, buf);
				n = 0;
				buf[0] = '\0';
			}
			n += snprintf (buf + n, sizeof(buf) - n, "\"%s\"", temp);
#endif
			buffer_data(file, buffer, &buffer_used_size, sizeof(buffer),
					"\"", sizeof("\"") - 1);
			buffer_data(file, buffer, &buffer_used_size, sizeof(buffer),
...
#if 0
			if(sizeof(buf) - n < strlen(cursor->text) + 1)
			{
				fprintf(file, buf);
				n = 0;
				buf[0] = '\0';
			}
			n += snprintf (buf + n, sizeof(buf) - n, cursor->text);
#endif
			buffer_data(file, buffer, &buffer_used_size, sizeof(buffer),
					cursor->text, strlen(cursor->text));
			goto state2;	/* close value */
			break;
	...
	//		fprintf (file, "true");
#if 0
			if(sizeof(buf) - n < strlen("true") + 1)
			{
				fprintf(file, buf);
				n = 0;
				buf[0] = '\0';
			}
			n += snprintf (buf + n, sizeof(buf) - n, "true");
#endif
			buffer_data(file, buffer, &buffer_used_size, sizeof(buffer),
					"true", sizeof("true") - 1);
			goto state2;	/* close value */
		...
		//	fprintf (file, "false");
#if 0
			if(sizeof(buf) - n < strlen("false") + 1)
			{
				fprintf(file, buf);
				n = 0;
				buf[0] = '\0';
			}
			n += snprintf (buf + n, sizeof(buf) - n, "false");
#endif
...
enum json_error json_insert_string (json_t * parent, const char *text_label, const char *text_value)
{
	return json_insert_pair_into_object(parent, text_label, json_new_string(text_value));
}

enum json_error json_insert_number (json_t * parent, const char *text_label, int value)
{
	char str[21] = {0};
	snprintf(str, 20, "%d", value);
	return json_insert_pair_into_object(parent, text_label, json_new_number(str));
}
enum json_error json_array_insert_string (json_t * parent, const char *text_value)
{
	return json_insert_child(parent, json_new_string(text_value));
}

enum json_error json_array_insert_number (json_t * parent, int value)
{
    char str[21] = {0};
	snprintf(str, 20, "%d", value);
	return json_insert_child(parent, json_new_number(str));
}

enum json_error json_array_insert_number_long (json_t * parent, long value)
{
	char str[32] = {0};
	snprintf(str, 31, "%ld", value);
	return json_insert_child(parent, json_new_number(str));
}
...
enum json_error JSON_INSERT_STRING(json_t * parent, const char *text_label, const char *text_value)
{
	if(text_value == NULL)
	{
		return json_insert_pair_into_object(parent, text_label, json_new_string(""));
	}
	else
	{
		return json_insert_pair_into_object(parent, text_label, json_new_string(text_value));
...
enum json_error JSON_ARRAY_INSERT_STRING(json_t * parent, const char *text_value)
{
	if(text_value == NULL)
	{
		return json_insert_child(parent, json_new_string(""));
	}
	else
	{
		return json_insert_child(parent, json_new_string(text_value));
...
int main()
{
    ...
    json_insert_number(json, "json_test", 123);


	json_stream_output(stdout, json);
